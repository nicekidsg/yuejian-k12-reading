...
import { books, categories, featuredBooks } from "./data/books";
    ...
    <article className="catalog-card">
      <button className="catalog-card-main" type="button" onClick={() => onOpen(book)}>
        ...
        <span className="catalog-copy">
          <span className="catalog-category">{book.category}</span>
      ...
      <footer>
        <span><ShieldCheck size={15} weight="fill" /> 公版原著</span>
...
function StarMap({ completedBooks, onExplore }) {
  ...
  return (
    <section className="reading-trail" aria-labelledby="trail-title">
      ...
      <div className="trail-achievement"><Star size={38} weight={completedBooks.size ? "fill" : "regular"} /><span><strong>{completedBooks.size ? "继续发光" : "等待第一束星光"}</strong><small>已读完 {completedBooks.size} / 100 本</small></span><ProgressBar value={completedBooks.size} label="全部馆藏完成进度" /></div>
    </section>
  );
}
  ...
  return (
    <div className="view-screen home-view">
      <section className="home-hero">
        <div><p className="eyebrow"><Sparkle size={18} weight="fill" /> 100 本免费英文原著</p><h1>从一本真正读得完的书开始</h1><p>所有书都可直接在站内阅读。收藏、进度与星图从 0 开始，只记录你的真实阅读。</p><button className="primary-button" type="button" onClick={() => onNavigate("探索")}>开始选书 <ArrowRight size={18} weight="bold" /></button></div>
      ...
      <section className="content-section">
        ...
        <div className="category-quick-grid">
          {categories.map((category) => { const Icon = categoryIcons[category.id]; return <button className={`category-quick tone-${category.tone}`} type="button" key={category.id} onClick={() => onExplore(category.id)}><span className={`catalog-symbol ${category.tone}`}><Icon size={29} weight="fill" /></span><span><strong>{category.name}</strong><small>{category.description} · 20 本</small></span><ArrowRight size={18} /></button>; })}
        </div>
      </section>
...
function ExploreView({ selectedTheme, setSelectedTheme, filterLevel, setFilterLevel, filteredBooks, visibleCount, setVisibleCount, savedBooks, readingProgress, onOpenBook, onToggleSaved, onExplore }) {
  ...
  return (
    <div className="view-screen explore-view">
      <section className="explore-hero">
        <div className="explore-copy"><p className="eyebrow"><Compass size={18} weight="fill" /> 探索真实书海</p><h1>100 本可直接阅读的英文故事</h1><p>精选自 Project Gutenberg 儿童文学书架。完整正文已内置，点击“开始阅读”即可进入。</p><div className="theme-pills">{categories.map((category) => { const Icon = categoryIcons[category.id]; return <button className={selectedTheme === category.id ? "is-active" : ""} type="button" key={category.id} onClick={() => onExplore(category.id)}><Icon size={18} weight="fill" />{category.name}</button>; })}</div></div>
      ...
      <section className="catalog-section" aria-labelledby="catalog-title">
        <header className="catalog-header"><div><p className="eyebrow">完整馆藏</p><h2 id="catalog-title">{selectedTheme === "全部" ? "全部 100 本" : selectedCategory?.name}</h2><p>难度为平台按篇幅估算，建议先试读再决定。</p></div><span className="catalog-count"><strong>{filteredBooks.length}</strong><small>本匹配</small></span></header>
        <div className="catalog-filters"><div className="filter-group"><strong>主题</strong><button className={selectedTheme === "全部" ? "is-active" : ""} type="button" onClick={() => { setSelectedTheme("全部"); setVisibleCount(20); }}>全部</button>{categories.map((category) => <button className={selectedTheme === category.id ? "is-active" : ""} type="button" key={category.id} onClick={() => { setSelectedTheme(category.id); setVisibleCount(20); }}>{category.name}</button>)}</div><div className="filter-group"><strong>难度</strong>{["全部", "入门", "进阶", "挑战"].map((level) => <button className={filterLevel === level ? "is-active" : ""} type="button" key={level} onClick={() => { setFilterLevel(level); setVisibleCount(20); }}>{level}</button>)}</div></div>
...
export function App() {
  ...
  const filteredBooks = useMemo(() => {
    ...
    return books.filter((book) => (!normalized || `${book.title} ${book.author} ${book.category} ${book.level} ${book.ageRange}`.toLowerCase().includes(normalized)) && (selectedTheme === "全部" || book.themeId === selectedTheme) && (filterLevel === "全部" || book.level === filterLevel));
  }, [query, selectedTheme, filterLevel]);
  ...
  return <div className="app-shell"><aside className="sidebar"><Logo /><nav className="side-nav">{navItems.map((item) => <NavButton key={item.label} item={item} active={activeView === item.label} onClick={() => navigate(item.label)} />)}</nav><div className="source-card"><ShieldCheck size={23} weight="fill" /><span><strong>免费公版原著</strong><small>来源可追溯 · 完整正文</small></span></div></aside><main className="main-content"><header className="topbar"><div className="mobile-brand"><Logo /></div><div className="search-wrap"><MagnifyingGlass size={22} /><input value={query} onChange={(event) => handleSearch(event.target.value)} placeholder="搜索书名、作者、年龄或主题" aria-label="搜索 100 本读物" />{query && <button className="clear-search" type="button" onClick={() => setQuery("")} aria-label="清空搜索"><X size={17} /></button>}</div><div className="top-badge"><Leaf size={19} weight="fill" /><span><strong>100 本</strong><small>免费阅读</small></span></div></header>{activeView === "首页" && <HomeView savedBooks={savedBooks} readingProgress={readingProgress} completedBooks={completedBooks} onNavigate={navigate} onOpenBook={setDetailBook} onExplore={explore} />}{activeView === "探索" && <ExploreView selectedTheme={selectedTheme} setSelectedTheme={setSelectedTheme} filterLevel={filterLevel} setFilterLevel={setFilterLevel} filteredBooks={filteredBooks} visibleCount={visibleCount} setVisibleCount={setVisibleCount} savedBooks={savedBooks} readingProgress={readingProgress} onOpenBook={setDetailBook} onToggleSaved={toggleSaved} onExplore={explore} />}{activeView === "我的书架" && <ShelfView savedBooks={savedBooks} readingProgress={readingProgress} onOpenBook={setDetailBook} onToggleSaved={toggleSaved} onNavigate={navigate} />}{activeView === "阅读足迹" && <TrailView completedBooks={completedBooks} readingProgress={readingProgress} onExplore={explore} onOpenBook={setDetailBook} />}{activeView === "好友" && <FriendsView identity={friendIdentity} setIdentity={setFriendIdentity} summary={friendSummary} showToast={showToast} />}<footer className="privacy-footer"><ShieldCheck size={16} weight="fill" /> 正文与配图来自 Project Gutenberg 公版资源。个人阅读详情保存在本机；好友功能仅同步昵称与汇总进度。</footer></main><nav className="mobile-nav">{navItems.map((item) => <NavButton key={item.label} item={item} compact active={activeView === item.label} onClick={() => navigate(item.label)} />)}</nav><BookDetail book={detailBook} saved={detailBook ? savedBooks.has(detailBook.id) : false} progress={detailBook ? readingProgress[detailBook.id] || 0 : 0} onClose={() => setDetailBook(null)} onToggleSaved={toggleSaved} onRead={openReader} />{readerBook && <Reader book={readerBook} progress={readingProgress[readerBook.id] || 0} onClose={() => setReaderBook(null)} onProgress={updateProgress} />}<div className={`toast ${toast ? "is-visible" : ""}`} role="status"><CheckCircle size={21} weight="fill" />{toast}</div></div>;
}
