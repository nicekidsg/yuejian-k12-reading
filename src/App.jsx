  ...
  return (
    <div className="view-screen explore-view">
      <section className="explore-hero">
        <div className="explore-copy"><p className="eyebrow"><Compass size={18} weight="fill" /> 探索真实书海</p><h1>{books.length} 本可直接阅读的英文故事</h1><p>精选自 Project Gutenberg 儿童文学与低龄绘本书架。完整正文已内置，点击“开始阅读”即可进入。</p><div className="theme-pills">{categories.map((category) => { const Icon = categoryIcons[category.id]; return <button className={selectedTheme === category.id ? "is-active" : ""} type="button" key={category.id} onClick={() => onExplore(category.id)}><Icon size={18} weight="fill" />{category.name}</button>; })}</div></div>
...
export function App() {
  ...
  const filteredBooks = useMemo(() => {
    ...
    return books.filter((book) => (!normalized || `${book.title} ${book.author} ${book.category} ${book.level} ${book.ageRange}`.toLowerCase().includes(normalized)) && (selectedTheme === "全部" || book.themeId === selectedTheme) && (filterLevel === "全部" || book.level === filterLevel) && (filterAge === "全部" || book.ageGroup === filterAge));
