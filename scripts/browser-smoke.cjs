...
(async () => {
  ...
  await firstCard.getByRole("button", { name: /收藏 Alice's Adventures/ }).click();
  ...
  assert.equal(await page.locator(".page-count strong").textContent(), "1");
  assert.equal(await page.locator(".trail-step").filter({ hasText: "奇幻童话" }).locator("small").textContent(), "1/5 本");
  await page.reload({ waitUntil: "networkidle" });
